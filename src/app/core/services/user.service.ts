...
export class UserService {
  private readonly _users$ = new BehaviorSubject<User[]>([...INITIAL_USERS]);
  ...
  getById(id: string): User | null {
    return this._users$.getValue().find((u) => u.id === id) ?? null;
  }

  getByEmail(email: string): User | null {
    return this._users$.getValue().find((u) => u.email.toLowerCase() === email.toLowerCase()) ?? null;
  }

  getByRole(role: UserRole): User[] {
    return this._users$.getValue().filter((u) => u.role === role);
  }

  getApprovedMentors(): User[] {
    return this._users$.getValue().filter(
      (u) => u.role === UserRole.Mentor && u.mentorApprovalStatus === MentorApprovalStatus.Approved && u.status !== 'suspended',
    );
  }
  ...
  add(user: User): void {
    ...
    this._users$.next([
      ...current,
      {
        ...
        joinDate: user.joinDate ?? new Date().toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }),
      },
    ]);
  }
  ...
  update(id: string, changes: Partial<User>): void {
    const current = this._users$.getValue();
    this._users$.next(current.map((u) => (u.id === id ? { ...u, ...changes } : u)));
  }
  ...
  setMentorApprovalStatus(id: string, mentorApprovalStatus: MentorApprovalStatus): void {
    this.update(id, { mentorApprovalStatus });
  }

  reset(): void {
    this._users$.next([...INITIAL_USERS]);
  }
}
